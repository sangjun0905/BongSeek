...
// ModelConfig config(model_dir); 실행하면 끝
...
ModelConfig::ModelConfig(const fs::path& model_dir) {
    const fs::path target = resolve_config_path(model_dir);
    if (!load(target)) {
...
fs::path ModelConfig::resolve_config_path(const fs::path& base) const {
    ...
    const fs::path dir = fs::weakly_canonical(base);
    if (!fs::is_directory(dir)) {
        throw std::runtime_error("[ModelConfig] Model directory not found: " + dir.string());
    }
