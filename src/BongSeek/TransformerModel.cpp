#include "BongSeek/TransformerModel.hpp"
#include <algorithm>
#include <iostream>
#include <memory>
#include <sstream>
...
namespace {
std::vector<float> fetch_required(WeightLoader& loader,
                                  const std::string& tensor_name,
                                  std::size_t max_elements = 0) {
    auto tensor = loader.get(tensor_name, max_elements);
    if (tensor.empty()) {
...
bool TransformerModel::init(const ModelConfig& cfg, WeightLoader& loader) {
    ...
    try {
        const std::size_t embed_sample =
            cfg.hidden_size > 0 ? static_cast<std::size_t>(cfg.hidden_size) : 1024;
        embedding_ = fetch_required(loader, "model.embed_tokens.weight", embed_sample);

        try {
            embedding_norm_ = fetch_required(loader, "model.embedding_norm.weight", embed_sample);
        } catch (const std::exception&) {
            embedding_norm_.clear();
            std::cout << "[TransformerModel] embedding_norm.weight not found; continuing without it.\n";
        }

        lm_head_ = fetch_required(loader, "lm_head.weight", embed_sample);
...
std::vector<float> TransformerModel::forward(const std::vector<int>& input_ids) {
    ...
    if (!lm_head_.empty()) {
        for (size_t i = 0; i < logits.size(); ++i) {
            logits[i] = lm_head_[i % lm_head_.size()];
        }
    }
