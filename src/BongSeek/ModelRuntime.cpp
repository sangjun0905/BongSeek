#include "BongSeek/ModelRuntime.hpp"
#include <algorithm>
#include <iomanip>
#include <iostream>
...
void ModelRuntime::demo_weights() {
    ...
    auto embed_weights = loader_.get("model.embed_tokens.weight", kSampleCount);
    auto head_weights = loader_.get("lm_head.weight", kSampleCount);
    std::cout << std::fixed << std::setprecision(3);

    if (!embed_weights.empty()) {
        std::cout << "  Embed sample: ";
        const size_t count = std::min<std::size_t>(embed_weights.size(), kSampleCount);
    ...
    if (!head_weights.empty()) {
        ...
        const size_t count = std::min<std::size_t>(head_weights.size(), kSampleCount);
        for (size_t i = 0; i < count; ++i) {
