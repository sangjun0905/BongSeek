...
inline std::shared_ptr<Variable> convld(
    ...
    int groups = 1) 
{
    auto f = std::make_shared<Convld>(stride, padding, groups);
    auto outs = (*f)({x, w});
    return outs[0];
}
...
public:
    ...
    std::shared_ptr<Variable> forward(const std::shared_ptr<Variable>& x) override {
        auto output = convld(x, W, stride_, padding_);
